...
{
    public class VipBusTrip : Trip
    {
        public VipBusTrip(string tripOrigin, string tripDestination, decimal ticketPrice, int capacity, int emptySeat, int busId) : base(tripOrigin, tripDestination, ticketPrice, capacity, emptySeat, busId)
        {
        }
        ...
        public override void ShowSeat()
        {
            ...
            for (int i = 1; i <= 10; i++)
            {
                for (int j = 1; j <= 3; j++)
                {
                    if (seatNumber == 16 || seatNumber == 17)
                    {
